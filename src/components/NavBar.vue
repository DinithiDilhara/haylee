<template>
  <nav class="h-16 flex items-center px-6 gap-4" style="background: #0D120E;">
    ...
    <span @click="$emit('openCart')" class="text-sm cursor-pointer" style="color: #E8E9E0;">
      Cart
    ...
    <button @click="themeStore.toggleDark()" class="text-sm" style="color: #C4C8AC;">
      {{ themeStore.isDark ? '☀️' : '🌙' }}
    </button>

    <button @click="handleLogout" class="text-sm" style="color: #C4C8AC;">
      Logout
    </button>
  </nav>
</template>
